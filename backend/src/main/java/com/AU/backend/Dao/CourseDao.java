...
public class CourseDao {
    ...
    public Course addCourse(Course course)throws Exception {
        ...
        return jdbcTemplate.queryForObject("select courseName,courseDescription,courseId,prerequisite,userId, createdOn, lastUpdated, imageUrl from course where courseId=" + keyHolder.getKey(), this::mapRowToCourse);
    }
    catch (Exception e)
    {
    ...
    public Course updateCourse(int courseId, Course course)throws Exception {
        ...
        try {
            ...
            return jdbcTemplate.queryForObject("select courseName,courseDescription,courseId,prerequisite,userId, createdOn, lastUpdated from course where courseId='" + courseId + "'", this::mapRowToCourse);
        }
        catch (Exception e){
            logger.error(e.getCause()+" in this method "+Thread.currentThread().getStackTrace()[1].getMethodName());
            throw e;
    ...
    {
        try{
            Course course=this.jdbcTemplate.queryForObject("select * from course where courseId="+courseId,this::mapRowToCourse);
    ...
    {
        try{
            List<Course> courses= this.jdbcTemplate.query("select * from course",this::mapRowToCourse);
